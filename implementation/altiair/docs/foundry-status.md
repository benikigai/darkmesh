# Palantir Foundry Status

Date: 2026-05-03

Using the signed-in Palantir browser, the backend-service application for the Pi-side CASK edge daemon now exists in Developer Console.
...
- Object API name visible in Palantir: `sampleCaskGpsPosition`.
- Relevant action type: `Create [Example] CASK GPS Position`.
- Action API name visible in Palantir: `create-example-cask-gps-position`.
...
- `FOUNDRY_UPLOAD_PROFILE=cask_gps_position`: narrow Palantir-compatible live smoke that writes only `LocationFix` records to `[Example] CASK GPS Position`.

For the current Palantir ontology, use:
...
- Installed the generated package locally outside Git from the Palantir package tarball.
