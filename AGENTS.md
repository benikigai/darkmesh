...
This repo is the public Vercel site for `darkmesh` by team Altair. The deeper implementation repo is `github.com/digitalnomd/altiair`; this repo should stay focused on the public demo surface, pitch artifacts, screenshots, and concise technical handoff material.
...
- The Vercel static site can show the narrative and fixture-backed dashboard. The true hardware path runs through the local node API on the Altair LAN.
...
If adding a hardware bench photo, place it at `assets/hardware/darkmesh-pis.jpg`. The CSS already references that path and falls back to a generated dark mesh surface if the file is absent.
